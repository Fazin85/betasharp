...
public class GuiControls : GuiScreen
{
    ...
    public GuiControls(GuiScreen parentScreen, GameOptions options)
    {
        ...
        {
            Children.Add(new GuiSmallButton(i, leftX + i % 2 * 160, Height / 6 + 24 * (i >> 1), 70, 20, _options.GetOptionDisplayString(i)));
        }

        Children.Add(new GuiOptionsSlider(SensitivityId, Width / 2 + 5, Height / 6 + 130, _options.MouseSensitivityOption, _options.MouseSensitivityOption.GetDisplayString(translations), _options.MouseSensitivityOption.Value).Size(125, 20));
        Children.Add(new GuiSmallButton(InvertMouseId, Width / 2 - 155, Height / 6 + 130, _options.InvertMouseOption, _options.InvertMouseOption.GetDisplayString(translations)).Size(125, 20));

        Children.Add(new GuiButton(ButtonDone, Width / 2 - 100, Height / 6 + 168, translations.TranslateKey("gui.done")));
        Text = translations.TranslateKey("controls.title");
        DisplayTitle = true;
    }
    ...
    protected override void ActionPerformed(GuiButton button)
    {
        for (int i = 0; i < _options.KeyBindings.Length; ++i)
        {
            Children[i].DisplayString = _options.GetOptionDisplayString(i);
        }
    ...
    protected override void KeyTyped(char eventChar, int eventKey)
    {
        if (_selectedKey >= 0)
        {
            ...
            Children[_selectedKey].DisplayString = _options.GetOptionDisplayString(_selectedKey);
            _selectedKey = -1;
        }
        else
        {
