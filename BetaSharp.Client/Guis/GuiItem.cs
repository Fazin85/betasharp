...
{
    public static void DrawHorizontalLine(int startX, int endX, int y, uint color)
    {
        if (endX < startX) (startX, endX) = (endX, startX);
        DrawRect(startX, y, endX + 1, y + 1, color);
    }

    public static void DrawVerticalLine(int x, int startY, int endY, uint color)
    {
        if (endY < startY) (startY, endY) = (endY, startY);
        DrawRect(x, startY + 1, x + 1, endY, color);
    }

    public static void DrawRect(int x1, int y1, int x2, int y2, uint color)
    {
    ...
    public static void DrawGradientRect(int right, int bottom, int left, int top, uint topColor, uint bottomColor)
    {
