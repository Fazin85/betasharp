...
public class RepeaterRenderer : IBlockRenderer
{
    public bool Render(IBlockAccess world, Block block, in BlockPos pos, Tessellator tess, in BlockRenderContext context)
    {
        ...
        // 1. Base Rendering
        var slabCtx = context with
        {
            EnableAo = false,
            UvRotateTop = (direction + 1) % 4
        };

        Helper.RenderStandardBlock(block, pos, world, tess, slabCtx);
        ...
        Helper.RenderTorchAtAngle(block, tess, new Vec3D(pos.x + staticTorchX, pos.y + torchVerticalOffset, pos.z + staticTorchZ), 0.0D, 0.0D, slabCtx);
        Helper.RenderTorchAtAngle(block, tess, new Vec3D(pos.x + delayTorchX, pos.y + torchVerticalOffset, pos.z + delayTorchZ), 0.0D, 0.0D, slabCtx);
        return true;
    }
}
