...
internal class BlockFence : Block
{
    ...
    public override bool canPlaceAt(CanPlaceAtCtx ctx) => ctx.Level.BlocksReader.GetBlockId(ctx.X, ctx.Y - 1, ctx.Z) == id ? true : !ctx.Level.BlocksReader.GetMaterial(ctx.X, ctx.Y - 1, ctx.Z).IsSolid ? false : base.canPlaceAt(ctx);
